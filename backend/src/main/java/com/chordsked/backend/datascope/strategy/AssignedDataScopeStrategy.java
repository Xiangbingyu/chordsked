...
public class AssignedDataScopeStrategy implements DataScopeStrategy {
    ...
    public String buildCondition(DataScopeUserContext userContext, String tableAlias, String scopeField) {
        ...
        if ("c".equals(tableAlias) && "id".equals(scopeField)) {
            String campusIdList = userContext.getAuthorizedCampusIds().stream()
                    .filter(campusId -> campusId != null && campusId > 0)
                    .map(String::valueOf)
                    .collect(Collectors.joining(", "));
            return campusIdList.isEmpty() ? "1 = 0" : tableAlias + ".id IN (" + campusIdList + ")";
        }
        ...
        if ("campus_id".equals(scopeField)) {
            return tableAlias + "." + scopeField + " IN (" + campusIdList + ")";
        }
